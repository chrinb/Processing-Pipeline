function [tone_onset_times,tone_onset_csplus,tone_onset_csminus,tone_onset_vector] = preprocess_findAllToneOnsetTimes(sessionData)
...
tracker = 1;
if strcmp(sessionData.daqdata.metadata.Experiment_type,'ADAAT')
    stimulus_duration = 0.5; % seconds
elseif strcmp(sessionData.daqdata.metadata.Experiment_type,'ADAA')
    stimulus_duration = 4; % seconds
end
fs = sessionData.daqdata.metadata.Sampling_rate_downsampled;
stimulus_duration_samples = fs*stimulus_duration;
...
while tracker<length(tone_onset)
    if tone_onset(tracker) == 1
        ...
        tracker = tracker + round(1.2*stimulus_duration_samples);
    else
        tracker = tracker + 1;
    end
end

tone_onset_vector = tone_onset_start;
