function roiArray = run(imArray, diameter)

imArray = imArray - mean(imArray,3);

% Get the default options for autosegmentation
ops = autosegment.suite2p.getOptions();
ops.splitROIs = getOr(ops, {'splitROIs'}, 1);
ops.diameter = getOr(ops, 'diameter', diameter);
...
roiArray = autosegment.suite2p.getRoiArray(stat, [Ly, Lx]);
