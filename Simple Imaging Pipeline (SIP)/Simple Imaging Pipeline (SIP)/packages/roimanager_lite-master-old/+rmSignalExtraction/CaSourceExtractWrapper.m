function roiArray = CaSourceExtractWrapper(Y, numRois, roiSize)
% Run calcium source extraction (Paninski) and return autodetected rois.
%
%   Inputs: 
%           Y - image stack as a 3D array
%           numRois - number of Rois to look for
%   Output
%           roiArray - array of RoI objects
%           

% Descripton of variable: 
%   A : sparse matrix (nPixels x nRois ), where each value is the pixel
%       intensity of each identified pixel of the Roi.
%   
%   C : Calcium activity of each neuron
%   b : spatial background. Looks like a mean image... But squares have
%       been removed where Active? neurons are found. Maybe squares are
%       "removed" for all found components.
%   f : temporal background. Default one timeseries. My guess: Bulk neuropil
%       excluding squares that ar removed
%   centers : center of rois.., [Y, X]!!


    %% Preparation of data and parameters
    
    % Why not to do a mean instead? Possibly because we want to find the
    % noise level? - EH
