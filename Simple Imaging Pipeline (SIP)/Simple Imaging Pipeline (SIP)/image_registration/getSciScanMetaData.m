...
%-- On recordings from OsloScope v2, some metadata is a vector [512 0] when
%only the first number is relevant. Remove all second vectors.
metadata.xpixels = metadata.xpixels(1);
metadata.ypixels = metadata.ypixels(1);
metadata.zPosition = metadata.zPosition(1);
metadata.zoomFactor = metadata.zoomFactor(1);

try
    metadata.piezoNumberOfPlanes = metadata.piezoNumberOfPlanes(1);
end
