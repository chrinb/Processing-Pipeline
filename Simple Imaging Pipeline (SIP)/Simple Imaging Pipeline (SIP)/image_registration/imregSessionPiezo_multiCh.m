...
% Written by Eivind Hennestad. Modified for piezo by Andreas Lande.

%-- Settings
if nargin < 2 || isempty(options)
    options.rigid = true; % Run rigid motion correction?
    options.nonrigid = false; % Run nonrigid motion correction?
    options.correct_chs = {'Green'}; % Color of channels to be corrected, fex {'Green', 'Red'}.
    options.red_corr = false; % Use red channel as reference for correcting green channel? 
end
...
%-- Register images
for curr_channel = 1:imagingInfo.nCh % For each recording channel
    
    ch = ch_sorted{2, curr_channel};
    ...
    for c = initFrames
        ...
        sessionFolder = getSessionFolder(sessionID);
        if strncmp(sessionID,'ID',2) % If the session folder is not following the common data structure it will be written "ID" in the beginning of the sessionID
        ...
        % Load temporary image array from correct channel and frames
        ...
        for plane_num = start_plane:number_of_imaging_planes
            ...
            if options.rigid % Run rigid correction
                ...
                if options.red_corr % Use RED channel to correct green
                    
                    if curr_channel == 1 % RED channel
                ...
                else % Correct RED and GREEN independent of each other
