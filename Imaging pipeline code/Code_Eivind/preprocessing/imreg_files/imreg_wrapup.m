...
% S Peron Oct. 2009
%
% [im_c E] = imreg_wrapup (im_s, im_t, dx, dy, dtheta, affmat, warp_parms, opt)
%
...
%
% As an alternative to dx, dy, dtheta, you can specify 3x3xn affmat - affine matrices -
%  of the standard format.  apply_affine is employed to apply them to images.  Clearly,
%  this implies *one per frame* at most, so n is number of frames.  If you give one, it
%  is applied to all frames.  If you specify dx, dy, dtheta and affmat, affmat is ignored.
%  Affine matrices are used to apply an aribtrary 2-d transform that preserves parallel
%  lines (see http://www.coranac.com/tonc/text/affine.htm).
%
% Another option is to specify a warpfield via warp_parms.  This is interpolated 
%   to all points using get_warp_image and indeed implemented therein.  Here, 
%  you can pass warp_parms_r structure, 1 per frame or 1 to do ALL frames with same, 
%  where warp_parms_r is as described in imreg_warp_via_subregion.m.
%
...
%
function [im_c E] = imreg_wrapup (im_s, im_t, dx, dy, dtheta, affmat, warp_parms, opt)
  % --- prelims:
  sim = size(im_s);
  sdx = size(dx);
	sam = size(affmat);
	...
	if ((sdx(2) == 1 & nframes > 1) | (sdx(2) == 1 & nframes == 1) | sum(sdx)==0)
	  piecewise = 0;
	end
	if (piecewise & sum(sam) > 0) % nonsensical condition - can't do piecewise off of affmat
	  disp('imreg_wrapup::warning - affine transofrm specified with piecewise registration ; defaulting to piecewise.');
	end

	% --- affine transform matrix stuff
	if (sum(sam) > 0)
	  if (sam(1) ~= 3 | sam(2) ~= 3)
		  disp('imreg_wrapup::affine matrices must be in 3x3 form, with 0 0 1 for final row to allow translation.  This will not be pretty...');
		end
		aa_opt.debug = 0; % no debugging
		aa_opt.interp = 0; % no interpolation -- FOR NOW
	end
  ...
  % --- im_c construction
	...
	if ( ~piecewise)
		...
		elseif (sum(sam) > 0) % --- Affine transform
      for f=1:nframes;
				if (opt.debug  >= 1) ;  waitbar(f/nframes,wb, 'imreg-wrapup::alignment'); end
				if (length(sam) == 3) % multiple matrices - one per frame
				  im_c(:,:,f) = apply_affmat(im_s(:,:,f), affmat(:,:,f), aa_opt);
				else % ONE matrix
				  im_c(:,:,f) = apply_affmat(im_s(:,:,f), affmat, aa_opt);
				end
			end
		elseif (length(warp_parms) > 0) % --- Warpfield transform
      for f=1:nframes;
				if (opt.debug  >= 1) ;  waitbar(f/nframes,wb, 'imreg-wrapup::alignment'); end
				if (length(warp_parms) > 1) % 1 per frame
			    im_c(:,:,f) = get_warp_image(im_s(:,:,f), warp_parms(f).X,warp_parms(f).Y); 
				else % ONE warpfield
			    im_c(:,:,f) = get_warp_image(im_s(:,:,f), warp_parms.X,warp_parms.Y); 
				end
			end
		end
...
%	disp('saving to ~/Desktop/imreg_wrapup.mat'); save('~/Desktop/imreg_wrapup.mat', 'dx_o', 'dy_o', 'dx_mf', 'dy_mf', 'dx_c', 'dy_c', 'opt');
      ...
      [dx_c_1 dy_c_1] = correct_displacement_vectors(dx_pre_c, dy_pre_c, opt.lE, 1, 1, 10, opt.debug);
			...
			E_base = imreg_wrapup_calculate_error(3, im_s, im_t, 1:nframes);
			E_p8 = imreg_wrapup_calculate_error(3, im_c_p8, im_t, 1:nframes);
			E_p9 = imreg_wrapup_calculate_error(3, im_c_p9, im_t, 1:nframes);
			E_1 = imreg_wrapup_calculate_error(3, im_c_1, im_t, 1:nframes);
			E_o = imreg_wrapup_calculate_error(3, im_c_o, im_t, 1:nframes);
			E_mf = imreg_wrapup_calculate_error(3, im_c_mf, im_t, 1:nframes);
  ...
  if (length(dtheta) > 0)
	...
	%E = imreg_wrapup_calculate_error(opt.err_meth, im_c, im_t, 1:nframes);
    E=0;

	% --- plot movie? for debugging
	if (opt.debug  >= 1) ;  delete(wb); end
	if (opt.debug >= 2)
    imreg_wrapup_plot_err_movie( im_s, im_t, im_c, dx, dy, sim, nframes, opt.debug)
	end
%
