function peakIndex_hit = plot_populationPeakResponses_indexed_ADAA(sessionData,previousPeakIndex_hit)
...
if nargin<2
    peakIndex_hit = peakIndex;
else
    peakIndex_hit = previousPeakIndex_hit;
end

for ROI = 1:numROIs
   sortedROIresponsesAfterPeak(ROI,:) = mean_responses(peakIndex_hit(ROI,2),:);
end
...
%imagesc(zScoreNormalize(sortedROIresponsesAfterPeak),[-2,3]);
ylabel('HIT');
xticks([]);
